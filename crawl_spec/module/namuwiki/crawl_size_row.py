...
def crawl_size_row(temp_dict, td):
    if td[0].text == "규격" and td[1].text != "한국" and td[1].text != "UMTS"and td[1].text != "크기" and td[1].text != "싱글" and td[1].text != "기본" and "가로" not in temp_dict:
        ...
        if len(xyz) > 2:
            temp_dict["가로"] = xyz[0]
            temp_dict["세로"] = xyz[1]
            temp_dict["두께"] = xyz[2].replace("mm","")
        
        if size.find("g") != -1:
            temp_dict["무게"] = re.search('[0-9. ]+g', size).group().replace("g","")
        elif td[2].text.find("g"):
            temp_dict["무게"] = re.search('[0-9. ]+g', td[2].text).group().replace("g","")
        ...
        if size.find("g") != -1:
            ...
            temp_dict["무게"] = re.search('[0-9. ]+g', td[3].text).group().replace("g","")
