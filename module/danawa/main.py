...
def parse_danawa(df_input, driver, crawl_data, a,b):
    for index in range(a,b):
        ...
        if hasxpath(driver,'//*[@id="productListArea"]/div[3]/ul/li[1]/div/div[1]/a[1]') == True:
            ...
            #일반 상세정보테이블
            if soup.select(".col-lg-4 .col-md-3 .border > .text-center .p13 .bg-blue .clr06 >.pt-1 .row .no-gutters"):
                ...
                for tr in trs:
                    ...
                    if len(th) == 0:
                        ...
                        if th[0].text == "제조회사":
                            ...
                            temp_dict["화면크기(인치)"] = re.search('\([0-9.]+(인치)?\)', td[0].text ).group().replace("인치","").replace("-","")
...
def URL(df_input, driver, crawl_data, a,b):
    for index in range(a,b):
        ...
        if hasxpath(driver,'//*[@id="productListArea"]/div[3]/ul/li[1]/div/div[1]/a[1]') == True:
            ...
            if str(href).find("go_link_goods") != -1:
                print(index,"잡았다")
                pass
            else:
                # driver.get(href)
