...
def naver_jungo(crawl_data,seller_list,i,driver):
    ...
    if pd.notnull(seller_list.iloc[i]['fsl_rs_naver_nickname']) and pd.notnull(seller_list.iloc[i]['fsl_rs_naver_id']):
        ####테스트중####테스트중####테스트중####테스트중####테스트중####테스트중####테스트중####테스트중
        ...
        naver_cafe_id_list = seller_list.iloc[i]['fsl_rs_naver_id'].split("|")
        url_cafe = """https://m.cafe.naver.com/CafeMemberProfile.nhn?cafeId=10050146&memberId="""
        for naver_cafe_id,naver_nick in zip(naver_cafe_id_list,naver_nick_list):
            # driver.get(url_naver_1+naver_nick+url_naver_2)####테스트중####테스트중####테스트중####테스트중####테스트중####테스트중
            driver.get(url_cafe+naver_cafe_id)
            # if hasxpath(driver,'//*[@id="articleList"]/ul/li[1]/div[1]/a[1]'): ####테스트중####테스트중####테스트중####테스트중
            if True:
                sell_count = 0    #현재 판매중인 물품수
                dealing_count = 0 #현재 판매중인 물품중 스마트폰으로 추정되는 갯수
                last_sell = ""    #가장최근 판매글 게시일
                
                #driver.find_element_by_xpath('//*[@id="articleList"]/ul/li[1]/div[1]/a[1]').click()####테스트중####테스트중####테스트중####테스트중
                is_withdrawal = scroll_to_end(driver)
                if is_withdrawal == 1: 
                    my_dict = {
                    "vendor_type" : "중고나라", 
                ...
                if sell_count == 0:
                    is_activate = 0
                elif datetime.today().date()-last_sell>=timedelta(days=14):
                    is_activate = 0
